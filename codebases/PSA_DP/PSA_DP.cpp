/*############################################################### 
 * ## MODULE: PSA_DP.cpp ## VERSION: 1.0 ## SINCE 2015-08-29 ##      
#################################################################
## Edited by MacVim
...
const double MATCH_SCORE = 0;
const double MISMATCH_SCORE = -2.2;
const double INSERTION_SCORE = -1.8;
const double DELETION_SCORE = -1.8;
...
bool isMatch1 (char DNA1, char DNA2) {
    ...
    if ((DNA1 == 'A' && DNA2 == 'T') || (DNA1 == 'C' && DNA2 == 'G')) 
        return true;
    else return false;
}
...
void smith_waterman (Sequence seqA, Sequence seqB, Plane& plane, Trace& trace) {
    ...
    for (int i = 0; i < nRow; i ++) {
        for (int j = 0; j < nCol; j ++) {
            ...
            if (i == 0 && j == 0) continue;
            if (i == 0 || j == 0) {
                plane[i][j].score = i * INSERTION_SCORE + j * DELETION_SCORE;
                plane[i][j].action = i>0?INSERTION:DELETION;
            ...
            for (int l = 1; j - l > 0 ; l ++) 
                del_score = max(del_score, plane[i][j-l].score + l * DELETION_SCORE);
            double ins_score = MIN_DOUBLE;
            for (int l = 1; i - l > 0 ; l ++) 
                ins_score = max(ins_score, plane[i-l][j].score + l * INSERTION_SCORE);
